...
export default function Attendance() {
  ...
  // Generate dynamic attendance data based on selection
  const attendanceLog = useMemo(() => {
    ...
    return {
      days,
      firstDay, // To offset the calendar grid
      logs: Array.from({ length: days }, (_, i) => {
        const dayOfWeek = (firstDay + i) % 7;
        const isWeekend = dayOfWeek === 0 || dayOfWeek === 6;

        // Deterministic pseudo-random status based on date
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        >
          ...
          <div className="grid grid-cols-7 gap-2">
            {/* Day Headers (Sun-Sat or Mon-Sun depending on locale, sticking to Mon-Sun for consistency) */}
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
              <div key={day} className="text-center text-xs font-bold text-muted-foreground pb-2">{day}</div>
            ))}

            {/* Empty slots for starting the month */}
