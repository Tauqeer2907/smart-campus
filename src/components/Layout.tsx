...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const studentNav: NavItem[] = [
    ...
    { label: 'Finance', path: ROUTE_PATHS.FINANCE, icon: CreditCard },
      ...
      >
        ...
        <div className="p-4 border-t border-white/5 space-y-2">
          <Button
            ...
            className="w-full flex items-center justify-start gap-3 text-xs text-muted-foreground hover:text-primary transition-colors"
          >
            <ShieldAlert className="w-4 h-4" />
            {!isSidebarCollapsed && <span>Switch Role (Demo)</span>}
          </Button>
